...
import ManagingProduct from './ManagingProduct';
import Table from './UserProduct';
...
function SiderBarMenu() {
  ...
  switch (isClick) {
    ...
    case 2:
      ...
      break;
    default:
      content = (<div className="flex items-center justify-center h-48 mb-4 rounded bg-gray-50 dark:bg-gray-800">
        ...
        <ul className="space-y-2 font-medium">
          <li>
            <button
              ...
              className="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 peer dark:text-white dark:hover:bg-gray-700"
            >
              <svg
            ...
            <ul
              id="dropdown-example"
              className={clsx("py-2 space-y-2", openDropdown == 1 ? "block" : "hidden")}
            >
              <li>
                <a
                  href="#"
          ...
          <li>
            <button type="submit" className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group"
              onClick={() => {
                authContextValue.handleLogout();
                if (confirm("Bạn có muốn đăng xuất?")) {
                  navigate("/");
                  console.log("Đã chọn đăng xuất");
                } else {
