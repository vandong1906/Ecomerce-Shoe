    ...
    <div className="group fixed bottom-6 end-6">
      <div className="mb-4 hidden flex-col items-center space-y-2 group-hover:flex">
        {speedDialButton.map((button, index) => (
          <IconButton key={index} iconPath={button.iconPath} label={button.label} />
        ))}
      </div>
      <button type="button" className="flex h-14 w-14 items-center justify-center rounded-lg bg-blue-700 text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
        <svg className="h-5 w-5 transition-transform group-hover:rotate-45" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 18">
