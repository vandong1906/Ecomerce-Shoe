...
function TotalProduct() {
    ...
    return <>
        <div className="flex h-full w-full">
            ...
            <div className={clsx("w-1/3 h-full items-center flex bottom-0 sticky ", state ? 'hidden' : 'block')}>
                <button type="submit"
                    className="text-xl font-bold text-red-500 hover:text-red-700"
                    onClick={() => setstate(!state)}
                >
            ...
            <div className="ListProduct grid grid-cols-4 gap-4">
                {
                    products?.map((product, index) => (
                        <Link key={index} to={clsx('/product?product=' + product.product_id)} className="text-center transition duration-0 hover:shadow-2xl hover:duration-700">
