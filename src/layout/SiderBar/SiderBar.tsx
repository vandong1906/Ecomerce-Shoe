...
function SiderBarMenu() {
  ...
  const toggleClick = (index: number) => {
    ...
    console.log("Toggling content for index:", index);
  };

  const toggleSidebar = () => {
    setIsSidebarVisible(!isSidebarVisible);
    console.log("Sidebar visibility toggled to:", !isSidebarVisible);
  };
  ...
  switch (isClick) {
    case 3:
      ...
      break;
    case 4:
      content = <Table />; // Ensure this component exists or adjust import
      break;
    default:
      content = (
       <div></div>
  ...
  return (
    <>{!isSidebarVisible && (
        <button
          ...
          className="fixed left-4 top-1/2 transform -translate-y-1/2 z-50 p-3 bg-gray-800 text-white rounded-full shadow-lg hover:bg-gray-700 transition duration-300"
      ...
      <div
        className={clsx(
          "fixed left-0 z-40 w-64 h-screen transition-transform duration-300",
          isSidebarVisible ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="h-full px-3 py-4  bg-white dark:bg-gray-900 border-0 shadow-2xl ">
          ...
          {isSidebarVisible && (
            <button
              onClick={toggleSidebar}
              className=" float-right left-64 top-4 z-50 p-2 bg-gray-800 text-white rounded-full shadow-lg hover:bg-gray-700 transition duration-300"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
      ...
      <div
        className={clsx(
          ...
          isSidebarVisible ? "sm:ml-64" : "ml-0"
        )}
      >
        {content}
      </div>
